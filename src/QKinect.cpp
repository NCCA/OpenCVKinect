...
//----------------------------------------------------------------------------------------------------------------------
bool QKinect::getRGB(cv::Mat &o_buffer)
{
...
//----------------------------------------------------------------------------------------------------------------------
bool QKinect::getDepthRaw(cv::Mat &o_buffer )
{
...
for( unsigned int i = 0 ; i < 640*480 ; ++i)
{
	...
	m_bufferDepthRaw.data[i]=depth[i];
	//m_bufferDepthRaw.data[i] = m_bufferDepthRaw.data[i] == 0 || m_bufferDepthRaw.data[i] > 1024 ? 0 : 255 - (unsigned char)(((float)m_bufferDepthRaw.data[i] / 1024.0f) * 255.0f);
	m_bufferDepthRaw16.data[i]=depth[i];
	...
	case 0:
		...
		m_bufferDepth.data[3*i+1] = 255-lb;
		m_bufferDepth.data[3*i+2] = 255-lb;
		break;
	case 1:
		m_bufferDepth.data[3*i+0] = 255;
		m_bufferDepth.data[3*i+1] = lb;
		m_bufferDepth.data[3*i+2] = 0;
		break;
	case 2:
		m_bufferDepth.data[3*i+0] = 255-lb;
